...
exports.acceptCollaborationRequest = acceptCollaborationRequest;
exports.rejectCollaborationRequest = rejectCollaborationRequest;
...
async function checkCompatibility(req, res) {
    try {
        ...
        const [userA, userB] = await Promise.all([
            User_1.User.findById(userId).select('compatibilityProfile username name'),
            User_1.User.findById(targetUserId).select('compatibilityProfile username name')
        ]);
        if (!userA || !userB) {
        ...
        const compatibility = (0, compatibility_1.computeCompatibility)(userA.compatibilityProfile, userB.compatibilityProfile);
        return res.json({
            success: true,
            data: {
                compatibility,
...
async function createCollaborationRequest(req, res) {
    try {
        ...
        const idea = await Idea_1.Idea.findById(ideaId).populate('author', 'username name');
        if (!idea) {
        ...
        await Idea_1.Idea.findByIdAndUpdate(ideaId, {
            $push: {
                collabRequests: {
                    requesterId: userId,
                    status: 'pending',
        ...
        await Promise.all([
            ...
            User_1.User.findByIdAndUpdate(receiverId, {
        ...
        const populatedRequest = await CollaborationRequest_1.CollaborationRequest.findById(collaborationRequest._id)
...
async function acceptCollaborationRequest(req, res) {
    try {
        ...
        const request = await CollaborationRequest_1.CollaborationRequest.findById(requestId)
        ...
        await Idea_1.Idea.findByIdAndUpdate(request.ideaId, {
            $set: {
        ...
        await User_1.User.findByIdAndUpdate(request.requesterId, {
            $push: {
                notifications: {
...
async function rejectCollaborationRequest(req, res) {
    try {
        const userId = req.user?.id;
        const { requestId } = req.params;
        const request = await CollaborationRequest_1.CollaborationRequest.findById(requestId)
...
async function getMyRequests(req, res) {
    try {
        ...
        const [sentRequests, receivedRequests] = await Promise.all([
            CollaborationRequest_1.CollaborationRequest.find({ requesterId: userId })
                .populate('ideaId', 'title mode')
                .populate('receiverId', 'username name avatarUrl')
                .sort({ createdAt: -1 }),
            CollaborationRequest_1.CollaborationRequest.find({ receiverId: userId })
                .populate('ideaId', 'title mode')
                .populate('requesterId', 'username name avatarUrl')
                .sort({ createdAt: -1 })
        ]);
...
async function markAllNotificationsSeen(req, res) {
    try {
        const userId = req.user?.id;
