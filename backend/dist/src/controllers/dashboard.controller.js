...
async function getDashboardStats(req, res) {
    try {
        ...
        // Get user's stats
        const user = await User_1.User.findById(userId).select('stats username name');
        if (!user) {
        ...
        const recentIdeas = await Idea_1.Idea.countDocuments({
            author: userId,
            createdAt: { $gte: sevenDaysAgo }
        });
        const recentProjects = await Project_1.Project.countDocuments({
            author: userId,
            createdAt: { $gte: sevenDaysAgo }
        });
        ...
        const trendingProjects = await Project_1.Project.find({
            visibility: 'public',
            isDraft: false
        })
        ...
        // Get recent ideas
        const recentIdeasList = await Idea_1.Idea.find({
            author: userId,
            isPublic: true
        })
            .sort({ createdAt: -1 })
        ...
        // Get recent projects
        const recentProjectsList = await Project_1.Project.find({
            author: userId,
            visibility: 'public'
        })
            .sort({ createdAt: -1 })
...
async function getWeeklyActivity(userId) {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const ideasCount = await Idea_1.Idea.countDocuments({
            author: userId,
            createdAt: { $gte: date, $lt: nextDate }
        });
        const projectsCount = await Project_1.Project.countDocuments({
            author: userId,
            createdAt: { $gte: date, $lt: nextDate }
        });
...
async function getUserActivity(req, res) {
    try {
        ...
        // Get ideas
        const ideas = await Idea_1.Idea.find({
            author: userId,
        ...
        const collabRequests = await CollaborationRequest_1.CollaborationRequest.find({
            $or: [
                { requesterId: userId },
                { receiverId: userId }
            ],
