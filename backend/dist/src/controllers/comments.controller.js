...
async function createComment(req, res) {
    try {
        ...
        if (normalizedTargetType === 'idea') {
            const idea = await Idea_1.Idea.findById(targetId);
            if (!idea) {
                return res.status(404).json({ success: false, error: 'Idea not found' });
            }
        }
        else if (normalizedTargetType === 'project') {
            const project = await Project_1.Project.findById(targetId);
            if (!project) {
        ...
        if (normalizedTargetType === 'idea') {
            await Idea_1.Idea.findByIdAndUpdate(targetId, {
                $inc: { commentCount: 1 }
            });
            ...
            await Project_1.Project.findByIdAndUpdate(targetId, {
...
async function deleteComment(req, res) {
    try {
        ...
        await Comment_1.Comment.deleteMany({ parentComment: commentId });
        // Delete the comment
        await Comment_1.Comment.findByIdAndDelete(commentId);
        // Update comment count on target
        if (comment.targetType === 'idea') {
            await Idea_1.Idea.findByIdAndUpdate(comment.targetId, {
                $inc: { commentCount: -1 }
            });
        }
        else {
            await Project_1.Project.findByIdAndUpdate(comment.targetId, {
                $inc: { 'metrics.comments': -1 }
            });
        }
        return res.json({
