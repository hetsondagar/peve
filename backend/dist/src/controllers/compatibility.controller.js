...
exports.getCompatibilityProfile = getCompatibilityProfile;
exports.getCompatibilityProfileByUser = getCompatibilityProfileByUser;
exports.setupCompatibilityProfile = setupCompatibilityProfile;
exports.updateCompatibilityProfile = updateCompatibilityProfile;
exports.checkCompatibility = checkCompatibility;
exports.getSkillSuggestions = getSkillSuggestions;
exports.getCompatibilityOptions = getCompatibilityOptions;
...
async function getCompatibilityProfile(req, res) {
    try {
...
async function getCompatibilityProfileByUser(req, res) {
    try {
...
async function setupCompatibilityProfile(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ success: false, error: 'Unauthorized' });
        const { skills, preferredRoles, availabilityHours, timeZone, preferredWorkHours, interests, goals, workStyle } = req.body;
...
async function updateCompatibilityProfile(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ success: false, error: 'Unauthorized' });
...
async function checkCompatibility(req, res) {
    try {
        ...
        const [userA, userB] = await Promise.all([
            User_1.User.findById(userId).select('compatibilityProfile'),
            User_1.User.findById(targetUserId).select('compatibilityProfile')
        ]);
        if (!userA || !userB) {
...
async function getCompatibilityOptions(req, res) {
    try {
        const options = {
            preferredRoles: [
                'Frontend Developer',
