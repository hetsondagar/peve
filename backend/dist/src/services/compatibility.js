...
function computeCompatibility(userA, userB, opts) {
    ...
    const reasons = [];
    if (skillOverlap > 0.3)
        reasons.push('Shared technical skills');
    if (roleOverlap > 0.3)
        reasons.push('Complementary roles');
    if (interestOverlap > 0.3)
        reasons.push('Similar interests');
    if (timeOverlap > 0.5)
        reasons.push('Good time overlap');
    if (workStyleCompatibility > 0.7)
        reasons.push('Compatible work style');
    if (opts?.pastCollab)
        reasons.push('Past collaboration');
...
function calculateTimeOverlap(userA, userB) {
...
function calculateWorkStyleCompatibility(workStyleA, workStyleB) {
    let compatibility = 0;
    let factors = 0;
