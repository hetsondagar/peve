...
exports.fetchGithubRepoActivity = fetchGithubRepoActivity;
exports.fetchGithubRepoAutofill = fetchGithubRepoAutofill;
exports.buildRepositoryInsights = buildRepositoryInsights;
...
function decodeReadmeBase64(content, encoding) {
    if (encoding !== 'base64')
        return content;
    return Buffer.from(content, 'base64').toString('utf-8');
}
function stripMarkdownRough(md) {
    return md
        .replace(/```[\s\S]*?```/g, ' ')
        .replace(/`[^`]+`/g, ' ')
        .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
...
function inferCategory(topics, description, languages) {
    ...
    if (languages.includes('Python'))
        return topics.length ? 'AI/ML' : 'Library';
    return 'Web Application';
}
function inferDifficulty(langCount, sizeKb, signals) {
    let score = 0;
    if (langCount >= 4)
        score += 2;
    else if (langCount >= 2)
...
function inferDevelopmentStage(archived, pushedAt, openIssues, stars) {
    if (archived)
        return 'completed';
    const days = (Date.now() - new Date(pushedAt).getTime()) / (1000 * 60 * 60 * 24);
    if (days > 365 && openIssues < 3 && stars > 50)
        return 'completed';
    if (days > 180)
        return 'ongoing';
    if (stars < 3 && openIssues < 2)
        return 'prototype';
    return 'ongoing';
}
async function tryFetchPackageDeps(owner, repo, token) {
    const names = ['package.json', 'requirements.txt', 'pyproject.toml', 'go.mod', 'Cargo.toml', 'composer.json'];
    const found = [];
    for (const name of names) {
        try {
            const data = await ghJson(`/repos/${owner}/${repo}/contents/${encodeURIComponent(name)}`, token);
...
async function fetchGithubRepoActivity(owner, repo, token) {
    const commitTimeline = [];
    const contributorLeaders = [];
    const commitLines = [];
    try {
        const commits = await ghJson(`/repos/${owner}/${repo}/commits?per_page=25`, token);
    ...
    try {
        const contribs = await ghJson(`/repos/${owner}/${repo}/contributors?per_page=12`, token);
...
async function fetchGithubRepoAutofill(repoUrl, token) {
    const parsed = parseGithubRepoUrl(repoUrl);
    if (!parsed)
        throw new Error('Invalid GitHub repository URL.');
    const repoJson = await ghJson(`/repos/${parsed.owner}/${parsed.repo}`, token);
    let readme = '';
    try {
        const readmeJson = await ghJson(`/repos/${parsed.owner}/${parsed.repo}/readme`, token);
        readme = decodeReadmeBase64(readmeJson.content, readmeJson.encoding);
    }
    catch {
        readme = '';
    }
    const langs = await ghJson(`/repos/${parsed.owner}/${parsed.repo}/languages`, token);
    ...
    let ci = false;
    try {
        const root = await ghJson(`/repos/${parsed.owner}/${parsed.repo}/contents`, token);
    ...
    const developmentStage = inferDevelopmentStage(repoJson.archived, repoJson.pushed_at, repoJson.open_issues_count, repoJson.stargazers_count);
    return {
        autofill: {
            title,
...
function buildRepositoryInsights(autofill, htmlUrl) {
    ...
    if (autofill.openIssues > 20)
        networkSignals.push('Active issue surface — evolving product');
    ...
    const peveScorePreview = Math.min(100, Math.round(28 + docQuality + diversity + community + breadth + signals * 0.5));
    const scoreRationale = 'Preview score blends documentation depth, stack breadth, community traction, and structural signals from public metadata only.';
    return {
        ...autofill,
        htmlUrl,
