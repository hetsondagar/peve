...
function buildFallbackIntelligence(
  ...
  if (!hasTestsSignal) actionLines.push('Publish test commands and scope (unit/integration/e2e) to improve reliability confidence.');
  if (!hasCiSignal) actionLines.push('Add CI workflow + status badge so delivery quality is visible without digging into code.');
  if (docsDepth < 1200) actionLines.push('Expand README architecture and operations sections (setup, module boundaries, deployment path).');
  if (issuePressure > 0.35) actionLines.push('Reduce issue pressure by triaging stale items and labeling roadmap vs defects.');
  if (!hasContainerSignal) actionLines.push('Document runtime topology (services, ports, data flow) even if no containerization is used.');
  ...
  return {
    ...
    score_rationale_ml:
      `Score interpretation: strong in ${strongest[0].replace(/_/g, ' ')} (${strongest[1]}), weaker in ${weakest[0].replace(/_/g, ' ')} (${weakest[1]}). Recommended next step: ${actionLines[0]}`,
